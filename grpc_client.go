...
func pbFileChunkSend(fpath string, pbFile *pb.File, stream pb.FileTransfer_StreamReceiveClient) error {
	...
	go func(stream pb.FileTransfer_StreamReceiveClient) error {
		...
		for {
			...
			if err != nil {
				PrintError("pbFileChunkSend:resp from server", err)
				break
			}

			DebugInfo("pbFileChunkSend:stream.Recv", resp.Status, " : ", len(chanChunkSendStatus))

			cs := <-chanChunkSendStatus
			if cs == -1 {
				DebugInfo("pbFileChunkSend:stream.Recv", "Stop with error")
				break
			}

			if cs == 1 {
				DebugInfo("pbFileChunkSend:stream.Recv", "Stop normally")
				break
			}
		}
		return nil
...
func ClientSendFiles() error {
	...
	filepath.Walk(SourceDir, func(fpath string, finfo fs.FileInfo, err error) error {
		...
		DebugInfo("ClientSendFiles: Sending", fpath)
